    ...
    row ? (
      <div className="flex flex-1 flex-col items-center justify-end">
        <div className="mb-2 text-center">
          <div className="animate-pop-in text-3xl">{medal}</div>
          <div className="font-display font-bold">{row.nickname}</div>
          <div className="font-display font-bold text-brand">
            {row.totalScore.toFixed(1)}
          </div>
        </div>
        <div className={`w-full rounded-t-xl bg-brand ${height}`} />
      </div>
    ) : (
      ...
      <div className="mx-auto mt-8 max-w-md">
        ...
        <ol className="space-y-1">
          {leaderboard.map((r) => (
            <li
              key={r.playerId}
              className="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-sm ring-1 ring-line"
            >
              <span>
                <strong className="mr-2">{r.rank}.</strong>
                {r.nickname}
              </span>
              <span className="font-display font-bold">{r.totalScore.toFixed(1)}</span>
            </li>
          ))}
        </ol>
      </div>
